...
> Superseding note: this historical plan predated the Ghostty-only
> shadow-terminal direction. Read any old paired Ghostty and restty wording as
> historical. Current architecture keeps `botster-core` backend-neutral,
> places the blessed core-side terminal backend in `botster-terminal-ghostty`,
> and treats restty as a web/client renderer only, not authoritative terminal
> infrastructure.
...
- Repo context inspected:
  - `README.md`
  - `Cargo.toml`
  - `docs/archive/plans/session-process-wire-protocol.md`
  - `docs/archive/plans/client-stream-contract.md`
...
- Preserve opaque snapshot round-trip behavior by wrapping bytes with typed metadata only; core must never parse or reinterpret host-owned snapshot internals.
...
- Add `botster-core-test-support` fake helpers for consumers to prove conformance without a concrete terminal backend.
- Add focused tests that exercise the exported engine/contract path, not just type construction.
- Update README boundary text or a repo doc only enough to state why concrete terminal backends are optional host/future dependencies rather than core dependencies.

Non-scope:

- No broad terminal emulator rewrite.
- No Ghostty vendoring, build scripts, submodule setup, WASM, or native parser
  dependency in this ticket. No restty client renderer work.
...
- Concrete host implementations may choose a terminal backend such as Ghostty
  through trait adapters or opaque payloads. restty is not such a core backend;
  it is a client renderer that may consume terminal state later through client
...
- Whether snapshot metadata should carry `rows`/`cols` only or also `format`/`version`. If a format field is added, keep it an opaque label such as `TerminalSnapshotFormat(String)` and avoid enum cases that bake concrete backend or renderer policy into core.
...
- `README.md`
  - Add a concise boundary note naming concrete terminal backends as host/future
    dependencies, not core dependencies, while keeping restty framed as client
    rendering only.
...
The fake implementation can be intentionally small: append UTF-8-lossy printable text for screen reads, preserve all raw bytes in normalized chunks, store rows/cols, and replay snapshots by restoring bytes/metadata without parsing them. This is enough to prove the boundary while leaving full terminal fidelity to concrete host adapters.
...
- The new terminal screen engine belongs inside that host-provided runtime adapter, not beside the data plane as an alternate router. A concrete TryBotster session runtime can back those trait methods with Ghostty later, while restty remains a client renderer and `botster-core-test-support` backs the core seam with the fake implementation.
...
- Pulling Ghostty or restty into `botster-core` would bake concrete backend or renderer policy into the reusable contract crate.
...
- Docs could overpromise current terminal backend wiring. State dependency
  status precisely: concrete host adapters may use Ghostty; restty is client
  rendering; core does not depend on either in this slice.
...
4. `plain_screen_read_uses_fake_runtime_without_terminal_backend_dependency`
   - Feed simple output, request screen text/state, and assert the fake path works without a concrete terminal backend or client renderer.
...
- No Ghostty imports, restty imports, or build steps in core.
...
- `botster-core` owns the terminal screen/snapshot boundary as opaque payload, synced screen-state, and lifecycle hook contracts; Ghostty belongs in a concrete host adapter, restty remains a client renderer, and neither is a core dependency.
