...
- Repo context loaded:
  ...
  - `docs/architecture/engine-command-surface.md`: `BotsterEngine` is the
    ...
    out of core.
  - `docs/archive/plans/session-process-wire-protocol.md`: existing frame-level
  ...
  - `docs/archive/plans/core-session-worker-engine.md`: existing `SessionWorkerEngine`
  ...
  - `docs/archive/plans/bound-pty-output-queues-and-surface-backpressure.md`: current
    local PTY reader path already has typed session-I/O backpressure and
    production-path proof requirements.
  - `docs/archive/plans/core-notification-session-inbox-primitives.md`: existing
...
- `docs/archive/plans/durable-session-worker-protocol-restart-contract.md`
  - This plan artifact.
