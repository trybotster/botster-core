...
| `teardown_bounds` | `try_write` is non-blocking. After 512 unsuccessful writes of the same head frame, or 512 host ticks where an accepted write stays Full/WouldBlock without completing, Core fails the subscription. The hard stop that ends Core adapter work is: remove the owner, call contractually non-blocking `close()`, drop the adapter, return from the host tick. ClientWorker creates no OS thread. A blocking `close()` is an illegal adapter. The published harness rejects it. Core does not leak a thread or `ManuallyDrop` the adapter. |
