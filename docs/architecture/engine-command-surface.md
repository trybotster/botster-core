...
| Bind terminal adapter | live attach generation plus `Box<dyn TerminalAdapter + Send>` | typed bind error or bound inventory row | `DefaultBotsterEngine::bind_terminal_adapter`, `CoreDaemon::bind_terminal_adapter` | Advanced host/adapter seam; not a prelude command |
| List terminal subscriptions | none | `Vec<TerminalSubscriptionRecord>` | `DefaultBotsterEngine::list_terminal_subscriptions`, `CoreDaemon::list_terminal_subscriptions` | Control-plane identity only |
