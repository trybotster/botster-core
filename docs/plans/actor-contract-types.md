...
| Session-I/O requests/events | New `src/actor.rs` or `src/actor/session_io.rs`; may add terminal summary structs to `src/session.rs` if they are pure session concepts | `SessionIoRequest`, `SessionIoEvent`, `TerminalModeSummary`, `TerminalColorProfile`, `PasteFileErrorReason`, `PreparedSnapshot` fields | New core summaries translated from old worker contract; keep terminal bytes/snapshots opaque but type routing/control fields |
| Transport ingress/egress | Existing `src/transport.rs` | Extend `TransportIngress` and `TransportEgress` to cover subscribe, unsubscribe, input, resize, snapshot, focus, heartbeat, terminal bytes, scrollback, process exit, attach state, mode/focus changes, binary, and `BoundaryJson` | Extend existing scaffold; transport may depend inward on session/client/boundary types |
...
- Whether terminal color/mode shapes should be fully enumerated now or represented as minimal summaries. Use typed summaries for stable control fields; use opaque bytes only for terminal payload data.
