...
{
    let snapshot_size = TerminalScreenSize::new(37, 111);
    let mutated_size = TerminalScreenSize::new(12, 44);
    let visible_text = "opaque snapshot visible text";
    let mut engine = TerminalScreenEngine::new(runtime);
    ...
    let output = engine.normalize_output(visible_text.as_bytes());
    assert!(matches!(
        output.output,
        Some(output) if output.bytes == visible_text.as_bytes()
    ));

    let captured = captured_snapshot(engine.capture_snapshot().snapshot);
    assert!(
        !captured.bytes.is_empty(),
        "opaque backend should capture non-empty snapshot bytes"
    );
    assert_eq!(captured.size, snapshot_size);
    assert_eq!(captured.format.as_deref(), expected_format);
    ...
    assert_eq!(
        ...
        mutated_size,
        "runtime should report mutated size before opaque restore"
    );

    let replayed = engine.replay_snapshot(captured);
    assert_eq!(
        replayed.hooks,
        vec![TerminalScreenHook::SnapshotReplayed {
            size: snapshot_size
    ...
    assert_eq!(restored.size, snapshot_size);
    assert!(
        restored.plain_text.contains(visible_text),
        "opaque backend should restore visible screen text; restored text: {:?}",
        restored.plain_text
    );
}
